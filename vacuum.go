...
type VacuumFilterI interface {
	Init(n int, m int, maxKickSteps int)
	Clear()
	Insert(ele uint64) bool
	Lookup(ele uint64) bool
	Delete(ele uint64) bool
	GetLoadFactor() float64
	GetFullBucketFactor() float64
}
...
func (v *SemiSortCuckooFilter) Insert(ele uint64) bool {
	// Apply hash transformation
	ele = HashUtilMurmurHash64(ele ^ 0x12891927)
...
func (f *SemiSortCuckooFilter) SetBucket(pos int64, store []int64) {
	...
	if roundDown(startBitPos, 64) == roundDown(endBitPos, 64) {
		...
		uint64Slice[roundDown(startBitPos, 64)/64+1] =
			((unit2 >> (writingUpperBound + 1)) << (writingUpperBound + 1)) +
				higherPart
	}
}
...
func (f *SemiSortCuckooFilter) Delete(ele uint64) bool {
	// Hash the element
	ele = HashUtilMurmurHash64(ele ^ 0x12891927)
...
func (f *SemiSortCuckooFilter) Lookup(ele uint64) bool {
	// Hash the element
	ele = HashUtilMurmurHash64(ele ^ 0x12891927)
